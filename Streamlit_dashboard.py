...
df_fin1 = df_fin1.rename(
    columns={
        "country": "Country",
        "amount_due": "Total sales (in $)",
        "Number_of_order": "Total orders",
    }
)
df_fin2 = df_fin2.rename(
    columns={
        "customerNumber": "Customer Number",
        "phone": "Phone Number",
        "Still_have_to_be_paid": "Customer's debt  ($)",
        "Proportion_of_credit_allows_already_reached": "Proportion of credit authorized already reached (in %)",
    }
)
...
# ======== Hide indexes : To erase the indexes when you display a table on the streamlit dashboard
...
if dash == "HR (Louis)":
    ...
    # Metrics of the selected employee (1-Total sales 2-Number of time he appears in the top2)
    ...
    col1.metric(
        "Total sales 💵",
        round(
            sum(HR_df2["Total_amount_of_money"][HR_df2["Employee_Name"] == employee])
        ),
    )
    col2.metric(
        "Number of times in top 2 🏆",
        HR_df2["Employee_Name"][
            (HR_df2["Employee_Name"] == employee) & (HR_df2["sales_rank"] <= 2)
        ].value_counts(),
    )
    ...
    wr1 = (
        "Have a look to "
        + employee
        + " monthly perfomances in comparison to our best employees 🥇 🥈 🥉"
    )
    st.write(wr1)
    ...
    tempo_df = HR_correspondingDates[
        (HR_correspondingDates["Employee_Name"] == employee)
        | (HR_correspondingDates["sales_rank"] <= 2)
    ][["Total_amount_of_money", "Employee_Name", "sales_rank", "month_year_bis"]]
    ...
    ax.legend(
        ...
        (str(employee), "1st employee of the month", "2nd employee of the month"),
    )
    ax.set_ylabel("Total sales (in $)", fontsize=14)
    ax.set_xlabel("Date", fontsize=14)
    ax.set_title(
        "Monthly sales of the selected employee in comparison to our best employees",
        fontsize=14,
    )
...
#=================================== Finance (Louis) =======================================
    ...
    with fig_col1:
        ...
        sns.barplot(
            x=df_fin1["Country"],
            y=df_fin1["Total sales (in $)"],
            order=df_fin1.sort_values("Total sales (in $)", ascending=False).Country,
            color="red",
        )
    ...
    with fig_col2:
        ...
        to_disp = (
            "But we still have $"
            + str(total_debt)
            + " to take back ! Where is our money then ?"
        )
        ...
        ordered_df = df_fin2.sort_values(by="Customer's debt  ($)", ascending=False)
        ax3.bar(
            x=ordered_df["Customer Number"].astype(str),
            height=ordered_df["Customer's debt  ($)"],
            color=my_cmap(
                ordered_df["Proportion of credit authorized already reached (in %)"]
                / 100
            ),
            label=True,
        )
    ...
    kpi3.metric(
        ...
        value=ordered_df["Proportion of credit authorized already reached (in %)"][
            ordered_df["Customer Number"] == selected_customer
        ],
    )
...
#=================================== Logistic (Joao) =======================================
    ...
    if fig_to_disp == "Total quantity ordered":
        ...
        plt.title(
            "Total orders for the most ordered products",
            loc="left",
            fontweight="bold",
            fontsize=10,
        )
        ...
        plt.bar(
            df_log["productName"],
            df_log["Average_quantity_orders_by_month"],
            color="red",
        )
        ...
        plt.bar(
            df_log["productName"], df_log["How_many_months_left_we_have"], color="red"
        )
        plt.title(
            "How many month we have stock", loc="left", fontweight="bold", fontsize=10
        )
        plt.xticks(rotation=90, fontsize=7)
        plt.yticks(fontsize=7)
...
#=================================== Sales (Mauricio) =======================================
    ...
    options = [
        "Sales by month for the year of 2021",
        "Sales by month for the year of 2022",
    ]
    ...
    if selected_optionsalespm == "Sales by month for the year of 2021":
        ...
        ax_Salesbymonth2021.bar(
            df_sales["month"][df_sales["year"] == 2021],
            df_sales["sales"][df_sales["year"] == 2021],
            color="red",
        )
        ...
        plt.bar(
            df_sales["month"][df_sales["year"] == 2022],
            df_sales["sales"][df_sales["year"] == 2022],
            color="red",
        )
    ...
    st.write(
        "We can observe very high orders during the last months of the year. Considering that the product being sold by our company performs very similarly to the products sold by the physical toy market it is expected that Christmas turns out to be a considerably profitable period of the year"
    )

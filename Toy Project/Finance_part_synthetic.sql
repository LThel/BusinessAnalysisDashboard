...
with Finance_per_customer as (select o.customerNumber, sum(od.quantityOrdered*od.priceEach) as Amount_that_have_to_be_paid, n.already_paid, (sum(od.quantityOrdered*od.priceEach) - n.already_paid) as Still_have_to_be_paid
from orders as o
...
select orders.orderNumber, orders.customerNumber, orders.status from orders where orders.orderNumber in (select customerNumber from customers where not exists (SELECT checkNumber FROM payments where payments.customerNumber = customers.customerNumber));